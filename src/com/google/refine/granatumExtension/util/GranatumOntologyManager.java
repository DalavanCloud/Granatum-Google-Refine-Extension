...
public class GranatumOntologyManager {
	...
	public Model readModel() {
		...
		try {
			...
			URL ontologyfile = new URL(
					"https://raw.github.com/gofranshukair/Granatum-Google-Refine-Extension/master/resources/ontologies/Granatum-02-12-11.owl");
			BufferedReader in;

			in = new BufferedReader(new InputStreamReader(ontologyfile.openStream()));

			// InputStream in =
			// this.getClass().getResourceAsStream("/ontologies/Granatum-02-12-11.owl");
			if (in == null) {
	...
	public ArrayList<Study> getStudyTypes() {
		...
		while (results.hasNext()) {
			...
			study
					.setStudyLabel(binding.get("label") != null ? binding.get(
							"label").toString() : binding.get("uri").toString()
							.substring(
									binding.get("uri").toString().lastIndexOf(
											"/") + 1));
			study.setStudyURI(binding.get("uri").toString());
			studyTypes.add(study);
		}
		System.out.println(studyTypes);
	...
	public ArrayList<StudyMetadataAttribute> getStudyAttributes(String studyuri) {
		...
		while (results.hasNext()) {
			...
			studyAttribute
					.setLabel(binding.get("label") != null ? binding.get(
							"label").toString() : binding.get("uri").toString()
							.substring(
									binding.get("uri").toString().lastIndexOf(
											"#") + 1));
